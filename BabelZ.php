...
/*
Plugin Name: BabelZ
Plugin URI: https://www.rswr.net/babelz-translation-widgets-wordpress-plugin/
...
if (!class_exists("BabelZ")) {
	...
	function printAdminPage() {
		...
		if ($_POST['BabelZ-G-AnalyticsA'] == "on") update_option('BabelZ_G_AnalyticsA', "checked=on");
		else update_option('BabelZ_G_AnalyticsA', "");
		$BabelZ_G_AnalyticsB = $_POST['BabelZ-G-AnalyticsB'];
		update_option('BabelZ_G_AnalyticsB', $BabelZ_G_AnalyticsB);
...
?>
<div class="wrap">
<h2><?php _e('BabelZ - Google Translate Widget - 1.1.5','BabelZ'); ?></h2>
<style type="text/css">
<!--
...
if (!function_exists("BabelZ_Widget")) {
	...
	$BabelZ_G_AnalyticsA = get_option('BabelZ_G_AnalyticsA');
	$BabelZ_G_AnalyticsB = get_option('BabelZ_G_AnalyticsB');
	    _e("
	    ...
	    ");
	if ($BabelZ_G_Auto == 'checked=on') {
	    _e("	autoDisplay: true,");
	}
	if ($BabelZ_G_Mult == 'checked=on') {
	    _e("	multilanguagePage: true,");
	}
	if ($BabelZ_G_AnalyticsA == 'checked=on' && $BabelZ_G_AnalyticsB != '') {
	    _e("	gaTrack: true,
			gaId: '".$BabelZ_G_AnalyticsB."',");
	}
	    _e("
	...
	// Show Promote Link
  	if (get_option('BabelZ_Prom') == "checked=on") _e('<p><a href="https://www.rswr.net/babelz-translation-widgets-wordpress-plugin/" title="BabelZ WordPress Plugin">BabelZ (WordPress Plugin)</a></p>');
    }
}
...
if (!function_exists("BabelZ_ap")) {
    function BabelZ_ap() {
	global $BabelZ_init;
	wp_enqueue_script('BabelZ-Main','/wp-content/plugins/babelz/js/BabelZ-main.min.js', array('jquery'), '1.0');
	
	if (!isset($BabelZ_init)) return;
	if (function_exists('add_menu_page')) add_menu_page('BabelZ', 'BabelZ', 9, basename(__FILE__), array(&$BabelZ_init, 'printAdminPage'), 'dashicons-translation');
    }
}

// Initialize Class
if (class_exists("BabelZ")) $BabelZ_init = new BabelZ();
